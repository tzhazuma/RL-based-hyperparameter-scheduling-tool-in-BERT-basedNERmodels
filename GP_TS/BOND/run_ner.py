...
from modelscope import (
    AutoModel as MSAutoModel, 
    AutoConfig as MSAutoConfig, 
    AutoTokenizer as MSAutoTokenizer, 
    snapshot_download
)

# 保留原有的transformers导入，但后面会替换使用
...
DISTILBERT_MODELS = ["distilbert-base-uncased", "distilbert-base-cased"]
CAMEMBERT_MODELS = ["camembert-base"]
XLM_ROBERTA_MODELS = ["xlm-roberta-base", "xlm-roberta-large"]

ALL_MODELS = BERT_MODELS + ROBERTA_MODELS + DISTILBERT_MODELS + CAMEMBERT_MODELS + XLM_ROBERTA_MODELS
...
def main():
    ...
    try:
        model_name_map = {
            ...
            "bert-large-uncased": "AI-ModelScope/bert-large-uncased",
            ...
            "roberta-large": "AI-ModelScope/roberta-large",
            "distilbert-base-uncased": "AI-ModelScope/distilbert-base-uncased",
            "distilbert-base-cased": "AI-ModelScope/distilbert-base-cased",
            "xlm-roberta-base": "AI-ModelScope/xlm-roberta-base",
            "xlm-roberta-large": "AI-ModelScope/xlm-roberta-large",
        }
        
        # 使用ModelScope下载模型
        model_id = model_name_map.get(args.model_name_or_path, args.model_name_or_path)
        ...
        # 加载配置
        if args.config_name:
            config = config_class.from_pretrained(args.config_name, num_labels=num_labels)
        else:
            # 从ModelScope下载的目录加载配置
            config = config_class.from_pretrained(model_dir, num_labels=num_labels)
        
        # 加载分词器
        if args.tokenizer_name:
            tokenizer = tokenizer_class.from_pretrained(args.tokenizer_name, do_lower_case=args.do_lower_case)
        else:
            # 从ModelScope下载的目录加载分词器
        ...
        logger.info(f"Successfully loaded model, tokenizer and config from ModelScope")
    except Exception as e:
        logger.warning(f"Error loading from ModelScope: {e}. Trying original method...")
        # 回退到原始的加载方法
