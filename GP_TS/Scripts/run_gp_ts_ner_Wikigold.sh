...
# 设置数据目录
DATA_DIR="../../BOND/dataset/Wikigold_distant"
# 如果目录不存在，尝试从BOND目录推断
if [ ! -d "$DATA_DIR" ]; then
  BOND_DIR=$(dirname "$RUN_NER_PATH")
  DATA_DIR="${BOND_DIR}/dataset/Wikigold_distant"
  echo "尝试使用推断的数据目录: $DATA_DIR"
fi
# 确认数据目录存在
if [ ! -d "$DATA_DIR" ]; then
  echo "警告: 找不到数据目录: $DATA_DIR"
  echo "请提供正确的数据目录路径，或确保dataset/Wikigold_distant目录存在于BOND目录下"
fi
...
OUTPUT_DIR="./outputs/gp_ts_ner_optimization/Wikigold"
N_TRIALS=100
RANDOM_TRIALS=20
N_EPOCHS=30
MAX_SEQ_LENGTH=128
