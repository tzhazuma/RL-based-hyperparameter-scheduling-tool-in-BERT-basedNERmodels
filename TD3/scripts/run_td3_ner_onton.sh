...
# 设置数据目录 (保持不变)
DATA_DIR="../../GP_TS/BOND/dataset/ontonotes5_distant"
if [ ! -d "$DATA_DIR" ]; then
  BOND_DIR=$(dirname "$RUN_NER_PATH")
  DATA_DIR="${BOND_DIR}/dataset/ontonotes5_distant"
...
OUTPUT_DIR="../outputs/ontonotes5" # 新的输出目录
N_TRIALS=500
RANDOM_TRIALS=15  # TD3初始随机探索的试验次数 (can be tuned)
N_EPOCHS=100       # NER模型每个trial的训练轮数
MAX_SEQ_LENGTH=128
