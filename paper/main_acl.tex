...
SAC (RL, 10 trials) & [TBD] & -- & -- \\
\bottomrule
\end{tabular}
\caption{Training $F_1$ scores (\%) across datasets and scheduling methods. GP-TS uses 100 trials for Webpage and 10 trials for Wikigold (each with 1--2 epochs). The static baseline uses 10--20 epochs of full training. GRPO and SAC experiments use 10 trials $\times$ 2 epochs on Webpage. [TBD] indicates result extraction pending; ``--'' indicates not yet evaluated.}
\label{tab:results-summary}
\end{table}

\subsection{Cross-Dataset Analysis}

Based on the results obtained, we observe several important patterns. On the Webpage dataset, GP-TS achieves a 0.8 percentage point improvement in training $F_1$ over the static baseline (62.7\% vs.\ 61.9\%), demonstrating that Bayesian exploration can identify better hyperparameter configurations even with single-epoch trials. GRPO achieves 45.0\% on Webpage, substantially below the static baseline, which we attribute to the 2-epoch trial budget being insufficient for the RL agent to learn meaningful scheduling policies. On Wikigold, TD3 (54.0\%) slightly exceeds the static baseline (53.0\%), while GP-TS (38.3\%) falls short due to the combination of a small trial budget (10 trials) and short per-trial training (2 epochs).
