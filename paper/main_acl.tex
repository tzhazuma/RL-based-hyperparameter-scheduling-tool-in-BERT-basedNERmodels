...
Table~\ref{tab:results-summary} presents a consolidated view of all experimental results obtained to date.
...
GP-TS (BO) & \textbf{62.7} & [TBD] & [TBD] \\
TD3 (RL) & [TBD] & 54.0 & [TBD] \\
SAC (RL) & [TBD] & -- & -- \\
GRPO (RL) & [TBD] & -- & -- \\
\bottomrule
\end{tabular}
\caption{Training $F_1$ scores (\%) across datasets and scheduling methods. [TBD] indicates experiments in progress; ``--'' indicates not yet scheduled. All results use roberta-base with distant supervision labels.}
\label{tab:results-summary}
\end{table}

\subsection{Cross-Dataset Analysis}

Based on the results obtained so far, we observe that adaptive scheduling shows promise for improving $F_1$ scores under noisy supervision. On the Webpage dataset, GP-TS achieves a 0.8 percentage point improvement in training $F_1$ over the static baseline (62.7\% vs.\ 61.9\%), demonstrating that even single-epoch Bayesian exploration can identify better hyperparameter configurations. On Wikigold, TD3 reaches 54.0\% $F_1$ through extensive exploration over 227 trials, slightly exceeding the static baseline of 53.0\%.
