...
\quad Static baseline (roberta-base) & 75.9 & 61.9 & 53.0 \\
\quad GP-TS tuned (ours) & -- & 62.7 & 38.3 \\
\quad TD3 tuned (ours) & -- & -- & 54.0 \\
\bottomrule
\end{tabular}
\caption{Comparison with state-of-the-art distantly supervised NER methods. All scores are entity-level micro-averaged F1 (\%). ``--'' indicates the method was not evaluated on that dataset. Our scheduling methods operate on the BOND Stage I training loop (roberta-base) without self-training or denoising, making them complementary to the denoising approaches above.}
